...
class Application
  ...
  def start_game
    ...
    puts " "
    puts " "
    puts "      Salut à toi jeune entrepeneur, amuses toi avec le jeux du morpion".colorize(:blue)
    puts "      Si t'es pas bête, tu connais déjà les règles".colorize(:yellow)
    puts "      Si t'es con, va sur Google".colorize(:green)
    puts "      Ehhhh zéééééé partie ! ".colorize(:red)
    puts " "
  end
  ...
  def ask_player1
    ...
    print "      JOUEUR 1 : ".colorize(:white)
    player1 = gets.chomp
    puts "      T'auras comme symbole le X (tmanière t'as pas le choix)".colorize(:white)
    puts " "
    return player1
  end
  ...
  def ask_player2
    ...
    print "      JOUEUR 2 : ".colorize(:white)
    player2 = gets.chomp
    puts "      Ben toi t'auras le O (si t'es pas content, on s'en fiche royalement)".colorize(:white)
    puts " "
    puts " "
    puts "      Aller on peut enfin commencer".colorize(:white)
    puts "      Appuie sur entrer gogolito".colorize(:white)
    gets.chomp
    return player2
  end
